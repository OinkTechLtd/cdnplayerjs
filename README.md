...
- `?embed=1` — скрыть левую панель и показать только плеер (удобно для встраивания).
...
Для локального ввода прямо в поле также поддерживается многострочный плейлист в формате `.m3u/.txt` (с `#EXTINF`, `Название|URL`, `Название,URL` или просто `URL`).
...
4. Получишь ссылку вида: `?embed=1#enc=...&k=...` (страница откроется без блока CDNPlayerJS, только плеер).
