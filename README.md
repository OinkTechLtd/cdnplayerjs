...
- `?embed=1` — показать только плеер (без блока CDNPlayerJS)
...
Важно: `.m3u`/`.txt` — это контейнер-список каналов/потоков. Сами потоки внутри обычно `m3u8`, `mp4` и т.п. То есть "трансляция" идёт из ссылок внутри плейлиста.
...
При открытии `#enc=...` страница автоматически переходит в режим только-плеера (без боковой панели).
